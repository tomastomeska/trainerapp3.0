...
if ($selectedPlanId > 0) {
    ...
    $planStmt->execute([$selectedPlanId, $coachId]);
    $plan = $planStmt->fetch();
    if ($plan) {
        $plans[] = $plan;
    }
...
$itemsByPlan = [];
if (!empty($plans)) {
    $planIds = array_values(array_map(static fn(array $row): int => (int)$row['id'], $plans));
...
<div class="title">Jídelníčky trenéra</div>
<div class="subtitle">Vygenerováno <?= date('d.m.Y H:i') ?></div>

<?php if (empty($plans)): ?>
<p class="muted">Nebyl nalezen žádný jídelníček pro tisk.</p>
<?php else: ?>
    <?php foreach ($plans as $plan): ?>
        ...
        <section class="plan-block">
            <div class="plan-head">
                <span><?= h((string)$plan['name']) ?></span>
                <span>Vytvořeno <?= formatDateTime((string)$plan['created_at']) ?></span>
            </div>
            <div class="plan-body">
